...
| Stash | `GitStashUtils.kt`, `GitStashTracker.kt`, `GitStashCache.kt` | push/list/apply/drop、reflog/log 格式解析、缓存刷新 | 已接入基础管理，并作为 Smart Update/Checkout 的保留层 |
...
| 单分支动作 | `plugins/git4idea/shared/resources/intellij.vcs.git.shared.xml` 的 `Git.Branch` 组与 `plugins/git4idea/backend/resources/intellij.vcs.git.backend.xml` 的 `Git.Branch.Backend` 组装 Checkout/Compare/Diff/Rebase/Merge/Worktree/Update/Push/Pull/Rename/Delete；`GitSingleRefActions.kt` 提供动态 ActionGroup，`GitBranchPopupActions.java` 按 local/remote/tag 包装启用条件 | `BranchSidebar.tsx` 按 ref 类型生成菜单；`log-panel.ts` 负责输入与确认；`GitClient` 和 `RepositoryController` 实现对应命令及仓库级操作队列 |
