...
describe('log filters', () => {
  it('maps UI filters to Git log query arguments', () => {
    const query = logQueryFromFilters({ text: 'fix', regex: true, caseSensitive: true, user: 'Ada', date: 'yesterday', path: 'src, test/a.ts' }, 'refs/heads/main', now);
    expect(query).toMatchObject({ text: 'fix', regex: true, caseSensitive: true, author: 'Ada', ref: 'refs/heads/main', paths: ['src', 'test/a.ts'] });
    expect(query.since).toBeTruthy();
    expect(query.until).toBeTruthy();
  });

  it('keeps stable user choices', () => {
    expect(emptyLogFilters).toEqual({ text: '', regex: false, caseSensitive: false, user: '', date: 'all', path: '' });
